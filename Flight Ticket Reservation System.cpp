...
const char l_destination[8][20]={"CHENNAI","HYDERABAD","MUMBAI","VIJAYAWADA","KOLATA","DELHI"};	//local Destination
...
class passenger{
	...
	protected:
		...
		char destination[30];
	public:		
		passenger(string f_name, string l_name, string phone_no, string add="N/A", string pass_no="N/A", int dd=00000000): bseats(50), eseats(200)
		{
			datedep=dd;
			firstname=f_name;
			lastname=l_name;
...
class booking:public passenger
{
	...
	bool ldest()
	{
		...
		again:
		cout<<endl<<"Enter your origin(using Captial letters): ";
		cin>>origin;
		cout<<endl<<"Enter your destination(using Captial letters): ";
		cin>>destination;
		...
		bool idest(string pass)
	{
		...
		void seats()
		{
			bus_eco:
				if(bus>0&&eco>0)
				{
					...
					switch(k)
					{
						case 49:    //ASCII VALUES 49(decimal)-- 1(ASCII VALUE)
						{
							cout<<endl<<"Business Class";
							cout<<endl<<"Enter the number of seats: ";
							cin>>NoOfSeats;
			                
							payment=(payment*NoOfSeats)+20000;
							if(NoOfSeats<=bus)
							{
							...
							else
							{
								...
								k = getch();
								if(k==49)         //ASCII VALUES 49(decimal)-- 1(ASCII VALUE)
								{
									NoOfSeats = bus;
					...
					case 50:          //ASCII VALUES 50(decimal)-- 2(ASCII VALUE)
						{
		...
		void Meals()
		{
			...
			n = getch();
			if(n==49)     //ASCII VALUES 49(decimal)-- 1(ASCII VALUE)
				mealtype="Vegetarian";
			else if(n==50)   //ASCII VALUES 50(decimal)-- 2(ACSII VALUE)
		...
		void Registration()
		{
			...
			if(k==49)             //ASCII VALUES 49(decimal)-- 1(ASCII VALUE)
			{
				...
				<<endl<<"Time: 4.00 PM"
				<<endl<<"Ticket: HF"<<tkt
				<<endl<<"Bill(IN INDIAN RUPPEES): "<<payment<<"/-"<<endl;				
				outfile.close();
			}
			else if(k==50)           // ASCII VALUES 50(decimal)-- 2(ASCII VALUE)
			{
				ofstream outfile;
				outfile.open("flight.txt");
				outfile<<"|TICKET INFORMATION:"<<endl<<endl
...
class about
{
	public:
		void Aboutus()
		{
			cout<<"The GLK AIRLINES was first touched by our airline in 2002. Since from 2008 onwards,\n Our Airlines is the world's large leading airline with 6 biggest airplanes.\nWe are at the Customers high service in cuisine, comfort and care.\nSo make a plan with GLK AIRLINES\nWe will give Best offers when you will choose our Airlines\n ."<<endl<<endl;
...
class complain
{
	...
	void type()
	{
		...
		if(choice==49)     //ASCII VALUES 49(decimal)-- 1(ASCII VALUE)
		{
			cout<<"Airport Handling: ";
			types = "Aiprort Handling: ";
		}
		else if(choice==50)    //ASCII VALUES 50(decimal)-- 2(ASCII VALUE)
		{
			cout<<"Baggage Delivery: ";
			types = "Baggage Delivery: ";
		}
		else if(choice==51)     //ASCII VALUES 51(decimal)-- 3(ASCII VALUE)
		{
			cout<<"Booking and Ticketing: ";
			types = "Booking and Ticketing: ";
		}
		else if(choice==52)      //ASCII VALUES 52(decimal)-- 4(ASCII VALUE)
		{
			cout<<"Other: ";
			types = "Other: ";
		}
...
int main()
{
	...
	system("COLOR 8E");
	cout<<"\n\t\t\t\t\t\t\tHCL PROJECT\n\t\t\t\t\t  FLIGHT TICKECT RESERVATION SYSTEM\n";
	cout<<"\nPress Enter to continue: ";
	getch();

	cout<<endl<<"\n\n\n\tWELCOME TO GLK AIRLINES"<<endl;
...
//				}
				...
				else if(choice==51)         //ASCII VALUES 51(decimal)-- 2(ASCII VALUE)
				{
					system("cls");
					goto back2;
				}
			...
			case 50:	//choice = 2           //ASCII VALUES 50(decimal)-- 2(ASCII VALUE)
				{
					cout<<">>>>>>>>>>ROUTES<<<<<<<<<<<\n";
					routes obj;
					ans = obj.print();
			...
			case 51:	//choice = 3               //ASCII VALUES 51(decimal)-- 3(ASCII VALUE)
				{
					cout<<">>>>>>>>>>COMPLAINT<<<<<<<<<<<\n";
					cout<<"Enter Your First Name: ";
					cin>>f_name;
			...
			case 52:	//choice = 4             //ASCII VALUES 52(decimal)-- 4(ASCII VALUE)
				{
					cout<<"\t\t>>>>>>>>>>ABOUT US<<<<<<<<<<<\n";
					about obj;
					obj.Aboutus();
			...
			case 53:   //choice=5                 //ASCII VALUES 53(decimal)-- 5(ASCII VALUE)
				{
					cout<<">>>>>>>>>>MANAGE<<<<<<<<<<<\n";
					Manage pass;
					chnge:
			...
			case 48:	//choice = 0            //ASCII VALUES 48(decimal)-- 0(ASCII VALUE)
				{
					exit(0);
				}
